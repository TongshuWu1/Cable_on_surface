...
DEFAULT_CONFIG = {
    ...
    "trackdlo_max_iter": 50,
    "trackdlo_preprocess_max_iter": 50,
    ...
    "max_prediction_step_m": 0.06,
    "fast_motion_recovery": True,
    "fast_motion_min_observed_ratio": 0.75,
    "fast_motion_recovery_alpha": 0.85,
...
def parse_args():
    ...
    parser.add_argument("--camera-brightness", type=int, default=config["camera_brightness"])
    parser.add_argument("--camera-contrast", type=int, default=config["camera_contrast"])
    parser.add_argument("--camera-hue", type=int, default=config["camera_hue"])
    parser.add_argument("--camera-saturation", type=int, default=config["camera_saturation"])
    parser.add_argument("--camera-sharpness", type=int, default=config["camera_sharpness"])
    parser.add_argument("--camera-gamma", type=int, default=config["camera_gamma"])
    parser.add_argument(
    ...
    parser.add_argument(
        "--camera-origin-view",
        action=argparse.BooleanOptionalAction,
        default=config["camera_origin_view"],
        help="Display the 3D scene in the current camera frame so the camera is at XYZ (0, 0, 0).",
    )
    ...
    parser.add_argument("--trackdlo-max-iter", type=int, default=config["trackdlo_max_iter"])
    parser.add_argument("--trackdlo-preprocess-max-iter", type=int, default=config["trackdlo_preprocess_max_iter"])
    ...
    parser.add_argument("--fast-motion-recovery", action=argparse.BooleanOptionalAction, default=config["fast_motion_recovery"])
    parser.add_argument("--fast-motion-min-observed-ratio", type=float, default=config["fast_motion_min_observed_ratio"])
    parser.add_argument("--fast-motion-recovery-alpha", type=float, default=config["fast_motion_recovery_alpha"])
    parser.add_argument("--fast-motion-max-step-m", type=float, default=config["fast_motion_max_step_m"])
...
def open_zed(args):
    ...
    apply_camera_controls(zed, args)
    return zed


def apply_camera_controls(zed, args):
    def safe_set(setting_name, value):
        if not hasattr(sl.VIDEO_SETTINGS, setting_name):
            print(f"Camera setting unavailable: {setting_name}")
            return

        setting = getattr(sl.VIDEO_SETTINGS, setting_name)
        try:
            status = zed.set_camera_settings(setting, value)
            print(f"Camera setting {setting_name}={value}: {status}")
        except Exception as exc:
            print(f"Could not set camera setting {setting_name}: {exc}")
    ...
    print("Applying ZED image settings...")
    safe_set("BRIGHTNESS", args.camera_brightness)
    safe_set("CONTRAST", args.camera_contrast)
    safe_set("HUE", args.camera_hue)
    safe_set("SATURATION", args.camera_saturation)
    safe_set("SHARPNESS", args.camera_sharpness)
    safe_set("GAMMA", args.camera_gamma)
...
def transform_points(transform, points, preserve_shape=False):
    ...
    return transform_points_matrix(matrix, points, preserve_shape)


def transform_points_matrix(matrix, points, preserve_shape=False):
...
def transform_vertices_matrix(matrix, vertices):
    vertices = np.asarray(vertices, dtype=np.float32)
    if vertices.ndim != 2 or vertices.shape[1] != 6 or len(vertices) == 0:
        return np.empty((0, 6), dtype=np.float32)

    output = np.ascontiguousarray(vertices.copy(), dtype=np.float32)
    output[:, :3] = transform_points_matrix(matrix, vertices[:, :3], preserve_shape=True)
    valid = np.all(np.isfinite(output[:, :3]), axis=1)
    return np.ascontiguousarray(output[valid], dtype=np.float32)
...
def main():
    ...
    print(
        ...
        f"auto camera controls {args.auto_camera_controls}, "
        f"image settings B/C/H/S/Sharp/Gamma "
        f"{args.camera_brightness}/{args.camera_contrast}/{args.camera_hue}/"
        f"{args.camera_saturation}/{args.camera_sharpness}/{args.camera_gamma}, "
        f"IMU required {args.imu_required}"
    )
    length_mode = (
    ...
    display_frame = "camera origin XYZ=(0,0,0)" if args.camera_origin_view else "ZED world frame"
    print(f"3D display frame: {display_frame}")
    ...
    if args.trackdlo:
        tracker = TrackDLOTracker(
            ...
            TrackDLOParams(
                ...
                max_iter=args.trackdlo_max_iter,
                preprocess_max_iter=args.trackdlo_preprocess_max_iter,
                ...
                fast_motion_recovery=args.fast_motion_recovery,
                fast_motion_min_observed_ratio=args.fast_motion_min_observed_ratio,
                fast_motion_recovery_alpha=args.fast_motion_recovery_alpha,
                fast_motion_max_step_m=args.fast_motion_max_step_m,
    ...
    if args.camera_origin_view:
        print("3D view uses the current camera frame; cable XYZ is reported relative to camera origin (0,0,0).")
    else:
    ...
    try:
        while viewer.is_available():
            if zed.grab(runtime) <= sl.ERROR_CODE.SUCCESS:
                ...
                if (
                    args.camera_origin_view
                    and latest_tracking_state == sl.POSITIONAL_TRACKING_STATE.OK
                ):
                    world_to_camera_matrix = inverse_transform_matrix(pose_transform)

                if should_show_fused and (
                    not args.camera_origin_view or world_to_camera_matrix is not None
                ):
                    if args.camera_origin_view:
                        env_vertices = transform_vertices_matrix(
                            world_to_camera_matrix,
                            display_fused_vertices,
                        )
                        env_label = "FUSED spatial map in camera frame"
                        env_frame = "camera"
                    else:
                    ...
                    if should_show_fused and args.camera_origin_view:
                        env_label = "LIVE depth fallback while camera pose is unavailable for fused map"
                    elif len(fused_vertices) > 0:
                        env_label = (
                ...
                display_frame = "camera" if args.camera_origin_view else env_frame
                if args.camera_origin_view:
                    cable_points_view = observations["X_t"]
                    cable_nodes_view = observations.get("Y_t", observations["Y_observed_t"])
                elif env_frame == "world" and latest_tracking_state == sl.POSITIONAL_TRACKING_STATE.OK:
                ...
                viewer.update_vertices(
                    env_vertices,
                    f"{env_label} | frame {display_frame} | {tracker_status}",
                )
                viewer.update_cable(
                    cable_points_view,
                    cable_nodes_view,
                ...
                if now - last_stats_time >= 1.0:
                    print(
                        ...
                        f"shown {env_label} {len(env_vertices)} frame {display_frame} | "
