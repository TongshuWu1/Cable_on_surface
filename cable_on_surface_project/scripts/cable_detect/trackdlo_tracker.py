...
class TrackDLOParams:
    ...
    fast_motion_recovery: bool = True
    fast_motion_min_observed_ratio: float = 0.75
    fast_motion_recovery_alpha: float = 0.85
    fast_motion_max_step_m: float = 0.12
...
class TrackDLOTracker:
    ...
    def step(
        ...
        recovered = self.recover_fast_motion(
            Y_prior,
            observed_nodes_xyz,
            observed_valid_nodes,
            visible_nodes_extended,
        ...
        if recovered is not None:
            ...
            observed_count = int(np.count_nonzero(observed_valid_nodes))
            self.status = (
                f"fast-motion recovery: {visibility_state}, observed "
                f"{observed_count}/{self.node_count}, "
                f"step {max_node_step(self.Y - previous_Y) * 1000.0:.0f}mm"
            )
            return self._result(X_t, observed_nodes_xyz, observed_valid_nodes)
        ...
        guide_nodes, sigma2_pre, _ = cpd_mct_registration(
            ...
            mu=self.params.mu,
            max_iter=self.params.preprocess_max_iter,
            tol=self.params.tol,
    ...
    def recover_fast_motion(
        ...
        if not self.params.fast_motion_recovery:
            return None
        if observed_nodes_xyz is None or observed_valid_nodes is None:
            return None

        observed_nodes_xyz = np.asarray(observed_nodes_xyz, dtype=np.float64)
        observed_valid_nodes = np.asarray(observed_valid_nodes, dtype=bool)
        if observed_nodes_xyz.shape != (self.node_count, 3) or len(observed_valid_nodes) != self.node_count:
            return None

        observed_ratio = float(np.count_nonzero(observed_valid_nodes)) / max(self.node_count, 1)
        if observed_ratio < float(self.params.fast_motion_min_observed_ratio):
            return None
        if len(visible_nodes_extended) >= self.params.min_visible_nodes:
            return None
        if visibility_state == "mostly visible":
            return None

        filled = fill_missing_nodes(observed_nodes_xyz, observed_valid_nodes)
        if filled is None:
            return None

        alpha = float(np.clip(self.params.fast_motion_recovery_alpha, 0.0, 1.0))
        if alpha <= 0.0:
            return None

        delta = alpha * (filled - np.asarray(Y_prior, dtype=np.float64))
        delta = clamp_node_displacements(delta, self.params.fast_motion_max_step_m)
        if max_node_step(delta) <= 1e-9:
            return None
        return np.asarray(Y_prior, dtype=np.float64) + delta
