...
class ZedDepthGLViewer:
    ...
    def _draw_overlay(self, width=None, height=None):
        ...
        frame_label = self.coordinate_frame.upper()
        origin_label = "camera origin (0,0,0)" if self.coordinate_frame == "camera" else "world frame"
        footer = (
            f"Frame: {frame_label} {origin_label}    "
            f"View: {self.view_mode.upper()}    Point size: {self.point_size:.1f}    "
            f"Depth range: {self.depth_max_m:.1f} m"
        )
